...
const getapiLinkForPage = (url, pageNumber) =>
  url.replace(/page=\d*/, `page=${pageNumber}`);

const createLinkHandler = (url, onLinkApi) => (pageNumber) =>
  onLinkApi(getapiLinkForPage(url, pageNumber));
...
function NeoBrowse({ neos, links, page, onSelectId, isFetching, onLinkApi }) {
  ...
  return (
    <div className="NeoBrowse">
      ...
      <div className="NeoBrowse__pagination">
        {getPagination(
          page.number,
          page.total_pages,
          createLinkHandler(links.self, onLinkApi)
        )}
      </div>
